...
//
//compile with gcc commands-host.c -lm -o commands-host
//For details see:
...
//   sprintf(TextLine, "S            - set step time in ms: %d ms", AdvanceRate);
...
//  sendport(32);
  ...
  while (1){
    ...
    if(KeyHit == 's'){
      ...
      MessageText("Type new step time: ", 1, MessageY, 0);
      gotoxy(23, MessageY);
      scanf("%d", &AdvanceRate);
      if(AdvanceRate == 0){
